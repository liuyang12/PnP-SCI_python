...
from apex import amp # [optional] automated mixed precision (AMP) training from NVIDIA apex
...
def	resume_training(argdict, model, optimizer):
    ...
    if argdict['resume_training']:
        ...
        if os.path.isfile(resumef):
            ...
            # [AMP load amp state_dict] automated mixed-precision training
            if argdict['fp16']:
...
# 		  training_params['step'])
    ...
    print("[epoch {: 2d}][{: 5d}/{: 5d}]    loss: {: 3.2f}.".\
...
def save_model_checkpoint(model, argdict, optimizer, train_pars, epoch):
    ...
    # [AMP save checkpoint] automated mixed-precision training
    if argdict['fp16']:
        save_dict = { \
...
def validate_and_log(model_temp, dataset_val, valnoisestd, temp_psz, writer, \
                     epoch, lr, logger, trainimg, gray_mode=False):
    ...
    with torch.no_grad():
        for seq_val in dataset_val:
            ...
            out_val = denoise_seq_fastdvdnet(seq=seqn_val, \
                                            ...
                                            windsize=temp_psz,\
                                            model=model_temp)
            # psnr_val += batch_psnr(out_val.cpu(), seq_val.squeeze_(), 1.) # no need to squeeze
            psnr_val += batch_psnr(out_val.cpu(), seq_val, 1.)
        psnr_val /= len(dataset_val)
        t2 = time.time()
        print("\n[epoch %d] PSNR_val: %2.2f, on %.2f sec" % (epoch+1, psnr_val, (t2-t1)))
    ...
    try:
        ...
        if epoch == 0:
            ...
            # Log training images
            N, FC, Ht, Wt = trainimg.size()
            C = 1 if gray_mode else 3
            img = tutils.make_grid(trainimg.view(-1, C, Ht, Wt), \
                                   nrow=FC//C, normalize=True, scale_each=True)
